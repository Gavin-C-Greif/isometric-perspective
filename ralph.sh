...
# Parse arguments
TOOL="cursor"  # Default to cursor headless agent
MAX_ITERATIONS=10
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  # Run the selected tool with shared instructions from CURSOR.md
  if [[ "$TOOL" == "amp" ]]; then
    OUTPUT=$(cat "$INSTRUCTIONS_FILE" | amp --dangerously-allow-all 2>&1 | tee /dev/stderr) || true
