...
7. Ensure all rows in the Release Smoke Suite, the High-Risk Interaction Matrix, and the Math Hardening Visual Matrix are `PASS`.
8. Run `npm run lint` and record the result in the Release Gate Checklist in `SMOKE-TEST.md`.
9. If a release artifact is being prepared, run `npm run build` and record the result in the same checklist.
10. Do not publish a release if any smoke scenario, high-risk matrix row, math hardening checklist section, or quality check fails.

## Evidence

Treat the completed run metadata + smoke suite + high-risk interaction matrix + `high-risk-harness.json` validation output + math hardening visual matrix + gate checklist in `SMOKE-TEST.md`, plus the current profiling record and long-session stress scenario in `PERFORMANCE-BASELINE.md`, as the release validation artifact for the version being shipped.
